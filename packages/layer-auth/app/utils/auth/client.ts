...
import type { emailOtpSignup } from '@packages/layer-auth/server/utils/auth/plugins/emailOtpSignup'
...
export function createAuthState (session: SessionRef) {
   ...
   * Triggers a session refresh first to ensure we have the latest state
   */
  async function waitForLoggedIn (): Promise<void> {
    // Trigger refetch to get latest session state from server
    await refresh()

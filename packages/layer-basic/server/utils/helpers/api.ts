 ...
 *
 * Supports generic type parameter for extended error codes in sub-packages.
 */
export class ApiError<T extends string = ErrorCode> extends Error {
  readonly statusCode: number
  readonly code: T
  readonly details?: unknown

  constructor (statusCode: number, code: T, details?: unknown) {
 ...
 * @param code - Error code from ERROR_CODES (or extended codes)
 * @param details - Optional additional error details
 */
export function throwApiError<T extends string = ErrorCode> (
  statusCode: number,
  code: T,
  details?: unknown,
): never {
  throw new ApiError<T>(statusCode, code, details)
}

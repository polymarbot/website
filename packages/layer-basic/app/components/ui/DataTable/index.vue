...
import type { DataTableProps as BaseDataTableProps, DataTableSortEvent } from 'primevue/datatable'
...
export interface DataTableProps extends /* @vue-ignore */ BaseDataTableProps {
  ...
  /** Current filter and sort state (controlled externally) */
...
const emit = defineEmits<{
  /** Emitted when filters (including sort state) change */
...
function buildFrozenShadowStyle (column: DataTableColumn): Record<string, string> {
  if (column.field === lastLeftFrozenField.value) {
    return { boxShadow: '4px 0 8px -4px rgba(0, 0, 0, 0.15)' }
...
function buildColumnStyle (column: DataTableColumn): Record<string, string> {
  ...
  return { ...style, ...buildFrozenShadowStyle(column) }
}
...
function getFilterValue (column: DataTableColumn): any {
  ...
  return props.filters[field] ?? null
}
...
function buildFilterMenuItems (column: DataTableColumn): MenuItem[] {
  ...
  items.push({
    ...
    command: () => emitFilterChange({ [filterField]: null }),
  })
  ...
  column.filter.options?.forEach(option => {
    items.push({
      ...
      command: () => emitFilterChange({ [filterField]: option.value }),
    })
  })
...
function applyFilter (column: DataTableColumn, hide: () => void) {
  ...
  emitFilterChange({ [field]: value === undefined ? null : value })
  hide()
}
...
function clearFilter (column: DataTableColumn, hide: () => void) {
  ...
  emitFilterChange({ [field]: null })
  hide()
}

function onSort (event: DataTableSortEvent) {
  // sortOrder=0 means removableSort third click → clear sort
  if (event.sortOrder === 0) {
    emitFilterChange({ sortBy: null, sortOrder: null })
  } else {
    emitFilterChange({ sortBy: event.sortField, sortOrder: event.sortOrder })
  }
}

function emitFilterChange (changes: Record<string, any>) {
  emit('update:filters', { ...(props.filters ?? {}), ...changes })
}

// Sort state derived from filters
const hasSortableColumns = computed(() => props.columns.some(c => c.sortable))
const currentSortField = computed(() => props.filters?.sortBy ?? undefined)
const currentSortOrder = computed(() => {
  const order = props.filters?.sortOrder
  return order !== null && order !== undefined ? Number(order) : undefined
})
</script>
...
<template>
  <PrimeDataTable
    ...
    :removableSort="hasSortableColumns"
    @sort="onSort"
  >
...
<style scoped>
:deep(.p-datatable-tbody > tr:not(.p-datatable-empty-message)) {
  transition: background-color 0.2s;
}

:deep(.p-datatable-tbody > tr:not(.p-datatable-empty-message):hover) {

...
import StrategyStatsModal from '~/pages/strategies/[id]/components/StrategyStatsModal/index.vue'
...
export interface StrategyDisplayProps {
  ...
  /** Optional max buy size to display after the amount */
  maxBuySize?: number
  ...
  symbols?: string
  /** Bot ID for filtering stats (optional) */
  botId?: string
}
...
const strategyUrl = computed(() => props.strategyId ? `/strategies/${props.strategyId}` : undefined)
...
<template>
  <div class="inline-flex items-center gap-2 text-[1em]">
    ...
    <!-- Amount and maxBuySize (optional) -->
    <span
      v-if="amount !== undefined || maxBuySize !== undefined"
      class="text-muted-color"
    >
      (<template v-if="maxBuySize !== undefined">
        {{ maxBuySize }}
      </template><template v-if="amount !== undefined && maxBuySize !== undefined">
        ·
      </template><template v-if="amount !== undefined">

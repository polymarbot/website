...
const expiresAtText = computed(() => {
  ...
  return formatDateTime(subscription.value.expiresAt)
})
...
async function handleModalConfirm (result: SubscriptionActionResult) {
  ...
  // Create charge and redirect to payment page
  isProcessingPayment.value = true

  try {
    const response = await request.post<{ hostedUrl: string }>(
      '/api/subscription/create-charge',
      { plan: result.targetPlan, billingCycle: result.billingCycle },
    )
        ...
        <div class="rounded-xl border border-surface bg-surface p-5">
          ...
          <div class="mb-5">
            <span
              ...
              :class="`
                text-${PlanSeverityMap[currentPlan]}
              `"
            >

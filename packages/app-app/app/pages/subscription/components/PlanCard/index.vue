...
const T = useTranslations('pages.subscription.components.PlanCard')
...
const monthlyEquivalent = computed(() => {
  ...
  return props.plan.yearlyPrice / 12
})

const yearlySavings = computed(() => {
  if (!isYearly.value || !isPaidPlan.value) return null
  return props.plan.price * 12 - props.plan.yearlyPrice
})
...
function getStatsAccessLabel (): string {
  if (props.plan.limits.statsAccess === StatsAccess.FULL) return T('statsAccess.full')
  if (props.plan.limits.statsAccess === StatsAccess.EXTENDED) return T('statsAccess.extended')
  if (props.plan.limits.statsAccess === StatsAccess.NONE) return T('statsAccess.none')
  return T('statsAccess.limited')
}

function hasFullStatsAccess (): boolean {
  return props.plan.limits.statsAccess === StatsAccess.FULL
}
</script>
  ...
  >
    ...
    <ul class="mb-6 flex-1 space-y-3">
      ...
      <li class="flex items-center gap-2 text-sm">
        ...
        <span
          class="font-medium"
          :class="isUnlimited(plan.limits.maxBuySize) ? 'text-success' : `
            text-color
          `"
        >
          {{ formatLimit(plan.limits.maxBuySize) }}
        </span>
      </li>
      <li class="flex items-center gap-2 text-sm">

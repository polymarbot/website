...
definePageMeta({
  title: 'pages.subscription.payments.name',
})

const T = useTranslations('pages.subscription.payments')
const { t } = useI18n()
const request = useRequest()
const { formatDate } = useDate()

// Fetch payment history for AsyncDataTable
async function fetchPayments (filters: AsyncDataTableFetchParams): Promise<AsyncDataTableFetchResult<SubscriptionPaymentItem>> {
  ...
  >
    ...
    >
      ...
      <template #billingCycle="{ row }">
        {{ t(`pages.subscription.billingCycle.${row.billingCycle}`) }}
      </template>

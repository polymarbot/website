...
definePageMeta({
  ...
  title: 'pages.strategies._id_.title',
})

const route = useRoute()
const T = useTranslations('pages.strategies._id_')
...
async function fetchData () {
  ...
  if (!isNew.value) {
    try {
      ...
      navigateTo('/strategies', { replace: true })
    }
    return
  }
...
const intervalOptions = [
  { label: getIntervalLabelWithMinutes(MarketInterval.M5), value: MarketInterval.M5 },
...
async function handleSave () {
  ...
  try {
    ...
    if (isNew.value) {
      ...
      navigateTo(`/strategies/${result.id}`, { replace: true })
    } else {
...
function generateStrategyName () {
  ...
  const nameParts = steps.map(step => {
    ...
    return `${prefix}${step.size}(${step.price})`
  })
    ...
    <div class="flex flex-col gap-6">
      ...
      <div class="flex flex-col gap-1">
        ...
        <StrategyStepsInput
          ...
          :maxBuySize="subscription.limits.value.maxBuySize"

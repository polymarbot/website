...
const T = useTranslations('pages.strategies._id_.components.TradeStepModal')
const { t } = useI18n()

const isEditing = computed(() => !!props.step)

// Tab options for side selection
const hasBuySteps = computed(() => props.steps.some(s => s.side === TradeSide.BUY))
const sideOptions = computed(() => [
  { label: T('side.buy'), value: TradeSide.BUY },
  { label: T('side.sell'), value: TradeSide.SELL },
])

// Sell requires buy steps to exist
const sellBlocked = computed(() => (formState.side as TradeSideType) === TradeSide.SELL && !hasBuySteps.value)
...
// Computed properties to convert between display format (0.01-100%) and internal format (1-10000)
const maxUnderlyingChangeDisplay = computed({
  get: () => formState.maxUnderlyingChange !== undefined ? formState.maxUnderlyingChange / 100 : undefined,
  set: (value: number | undefined) => {
    formState.maxUnderlyingChange = value !== undefined ? Math.round(value * 100) : undefined
  },
})

const minUnderlyingChangeDisplay = computed({
  get: () => formState.minUnderlyingChange !== undefined ? formState.minUnderlyingChange / 100 : undefined,
  set: (value: number | undefined) => {
    formState.minUnderlyingChange = value !== undefined ? Math.round(value * 100) : undefined
  },
})

// Max sell size: total buy size of all existing steps
const maxSellSize = computed(() => getTotalBuySize(props.steps))
...
const schema = computed(() => {
  const baseSchema = {
    ...
    price: z.number()
      .min(0.01, t('pages.strategies._id_.components.TradeStepModal.validation.priceMin'))
      .max(0.99, t('pages.strategies._id_.components.TradeStepModal.validation.priceMax')),
    size: z.number()
      .min(5, t('pages.strategies._id_.components.TradeStepModal.validation.sizeMin')),
  ...
  if (formState.side === TradeSide.BUY) {
    return z.object({
      ...
      // Internal format: 1-10000 (0.01% - 100%)
      maxUnderlyingChange: z.number().min(0).max(10000).optional(),
      minUnderlyingChange: z.number().min(0).max(10000).optional(),
    }).refine(data => data.start < data.end, {
      message: t('pages.strategies._id_.components.TradeStepModal.validation.timeRange'),
      path: [ 'end' ],
    })
  }
...
watch(visible, isVisible => {
  if (isVisible) {
    if (props.step) {
      ...
      // Default to sell side if buy steps already exist
      if (hasBuySteps.value) {
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <!-- Sell blocked warning -->
      <Message
        v-if="sellBlocked"
        severity="warn"
      >
        {{ T('warnings.sellNeedsBuySteps') }}
      </Message>
      ...
      <div class="flex flex-col gap-1">
        ...
        <InputNumber
          ...
          name="size"
          :min="5"
          :max="(formState.side as TradeSideType) === TradeSide.SELL ? maxSellSize : undefined"
          :invalid="!!validationResult.size"
        />
        <small class="text-muted-color">
          {{ (formState.side as TradeSideType) === TradeSide.SELL
            ? T('hints.sellSizeRange', { max: maxSellSize })
            : T('hints.minSize')
          }}
        </small>
        <FormError

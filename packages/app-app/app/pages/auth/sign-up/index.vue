...
const { sendSignUpOtp, verifySignUpOtp, signInWithOAuth, resolveOAuthError } = useAuthActions()
...
async function sendCode () {
  ...
  // Validate form
  const fieldsToValidate: (keyof typeof formState)[] = [ 'email' ]
  if (inviteCodeRequired || formState.inviteCode) {
    fieldsToValidate.push('inviteCode')
  }
  try {
    await validate(fieldsToValidate)
  } catch {
    return
  }
            ...
            >
              ...
              <i18n-t keypath="pages.auth.sign-up.agreeToTerms">
                <template #termsOfService>
                  <WebLink
                    :href="homepageUrl('/legal/terms-of-service')"
                    target="_blank"
                  >

...
/**
 * Calculate the total size of all buy steps in a strategy.
 *
 * @param strategy - Strategy configuration (array of trade steps)
 * @returns Total buy size (sum of all buy step sizes)
 */
export function calculateStrategyMaxBuySize (strategy: StrategyConfig): number {
  return strategy
    .filter((step): step is TradeStepBuy => step.side === TradeSide.BUY)
    .reduce((sum, step) => sum + step.size, 0)
}

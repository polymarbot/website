...
export default defineWrappedResponseHandler(async event => {
  ...
  const maxBuySize = calculateStrategyMaxBuySize(normalizedStrategy)

  // Validate subscription limits (count + buy size)
  const ctx = createSubscriptionContext(user.id)
  await Promise.all([
    validateStrategyCount(user.id, 1, ctx),
    validateStrategyBuySize(user.id, maxBuySize, ctx),
  ])
  ...
  const strategy = await appDb.strategy.create({
    data: {
      name,
      interval,

...
export default defineWrappedResponseHandler(async event => {
  ...
  if (strategyJson !== undefined) {
    ...
    const newMaxBuySize = calculateStrategyMaxBuySize(normalizedStrategy)
    await validateStrategyBuySize(user.id, newMaxBuySize)
    ...
    const existingStrategy = await appDb.strategy.findFirst({
      where: {
        ownerId: user.id,
        strategyHash,
    ...
    updateData.maxBuySize = newMaxBuySize
  }

  // Update strategy
  const updatedStrategy = await appDb.strategy.update({
    where: { id },
    data: updateData,
    select: {
      id: true,
  ...
  const enabledBotCount = await appDb.bot.count({
    where: {
      strategyId: id,
      enabled: true,
    },
  ...
  if (enabledBotCount > 0) {
    await invalidateBotsCacheByIntervals([ strategy.interval ])
  }

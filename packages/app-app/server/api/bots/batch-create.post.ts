...
export default defineWrappedResponseHandler(async event => {
  ...
  const existingBots = await appDb.bot.findMany({
    where: {
  ...
  // Validate subscription limits (bot count + strategy buy size)
  if (symbolsToCreate.length > 0) {
    ...
    await Promise.all([
      ...
      validateStrategyBuySize(user.id, strategy.maxBuySize, ctx),
    ])
  }
  ...
  if (symbolsToCreate.length > 0) {
    const createdBots = await appDb.bot.createManyAndReturn({

 ...
 * Checks:
 * - Strategy buy size is within user's subscription limit
...
export default defineWrappedResponseHandler(async event => {
  ...
  // Validate strategy buy size against subscription limit
  await validateStrategyBuySize(user.id, bot.strategy.maxBuySize)
  ...
  const enabledBots = await appDb.bot.findMany({
    where: {
      funder: bot.funder,
      enabled: true,
    },
  ...
  if (process.env.APP_ENV === 'prod') {
    ...
    const balance = await getUSDCeBalanceCached(bot.funder)
    const totalRequiredAmount = occupiedAmount + Number(bot.strategy.amount)
    const requiredBalance = parseUnits(String(totalRequiredAmount * botBalanceMultiplier), USDCE_DECIMALS)
  ...
  if (bot.wallet.status === WalletStatus.INACTIVE || bot.wallet.status === WalletStatus.FAILED) {
    await triggerEnableTrading(bot.wallet, async () => {
      ...
      // Invalidate bots cache for this interval
      await invalidateBotsCacheByIntervals([ bot.interval ])

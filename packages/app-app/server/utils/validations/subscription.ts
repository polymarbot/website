...
export async function getUserSubscription (
  ...
  const subscription = await appDb.userSubscription.findUnique({
    where: { userId },
...
export async function getUserResourceCounts (
  ...
  const [ walletCount, strategyCount, botCount ] = await Promise.all([
    appDb.wallet.count({ where: { ownerId: userId, deleted: false }}),
    appDb.strategy.count({ where: { ownerId: userId }}),
    appDb.bot.count({ where: { wallet: { ownerId: userId, deleted: false }}}),
  ])
...
/**
 * Validate strategy maxBuySize against plan limit
 */
export async function validateStrategyBuySize (
  userId: string,
  maxBuySize: number,
  ctx?: SubscriptionContext,
): Promise<void> {
  ...
  if (limits.maxBuySize !== null && maxBuySize > limits.maxBuySize) {
    throwApiError(403, ERROR_CODES.SUBSCRIPTION_STRATEGY_BUY_SIZE_EXCEEDED, {
      maxBuySize,
      limit: limits.maxBuySize,
      plan: subscription.plan,
    })
  }
}
